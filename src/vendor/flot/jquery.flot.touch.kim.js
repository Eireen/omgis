...
(function($) {
    ...
    function init(plot) {
        ...
        function scale(delta, center) {
            var options = plot.getOptions();
            var plotOffset = plot.offset();

            var zoomMethod = delta < 0 ? 'zoomOut' : 'zoom';

            plot[zoomMethod]({
                center: {
                    left: center.x - plotOffset.left,
                    top: center.y - plotOffset.top,
                },
                amount: options.touch.zoom.amount,
            }); 
        }
        ...
        function bindEvents(plot, eventHolder) {
            ...
            placeholder.bind('touchend', function(evt) {
                    ...
                    startTouchPosition.y <= lastTouchPosition.y + options.touch.tapPrecision)
                {
                    ...
                    /* NOTE: evt приходится передавать в параметрах события, т.к. иначе плагин `flot.tooltip` получает
                    пустые originalEvent и touches при тестировании тапов на эмуляторе в декстопном браузере (Vivaldi) */
                    if (tapNum === 2) { 
                        placeholder.trigger("dbltap", [ evt, lastTouchPosition ]);
                    } else { 
                        placeholder.trigger("tap", [ evt, lastTouchPosition ]);
                    }
    ...
    $.plot.plugins.push({
        ...
        options: {
            touch: {
                zoom: {
                    amount: 1.1,        // custom value for jquery.flot.navigate.js
                },
                autoWidth: false,
                autoHeight: false,
